...
def test_gcs_model_registry_writes_versioned_and_production_artifacts() -> None:
    ...
    assert uris["manifest"] == (
        "gs://stock-analysis-medallion-prod/models/runs/train-run-1/manifest.json"
    )
    assert uris["production_current"] == (
        "gs://stock-analysis-medallion-prod/models/production/current.json"
    )
...
def test_gcs_model_registry_rejects_incomplete_manifest_bundle() -> None:
    client = FakeStorageClient()
    artifact = MLForecastModelArtifact(
        model=ConstantModel(0.01),
        model_version="lightgbm_return_zscore",
    ...
    bucket = client.buckets["stock-analysis-medallion-prod"]
    bucket.blobs["models/runs/train-run-1/metadata.json"].content = b""

    assert not registry.exists(registry.default_model_uri())
    try:
        registry.load_artifact(registry.default_model_uri())
    except ValueError as exc:
        assert "references missing artifact metadata" in str(exc)
    else:  # pragma: no cover
        raise AssertionError("expected ValueError")
    assert uris["production_current"] == registry.default_model_uri()

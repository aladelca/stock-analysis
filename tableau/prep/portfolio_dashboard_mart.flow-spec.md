...
1. Start from `optimizer_input.csv` so the mart contains every eligible ticker.
2. LEFT JOIN `portfolio_recommendations.csv` on `ticker`.
...
8. Add calculated fields:
   - `selected = NOT ISNULL([target_weight]) AND [target_weight] >= 0.001`
...
```text
one row per eligible ticker per run
```
...
- Row count equals `optimizer_input` row count.
